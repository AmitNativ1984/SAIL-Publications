...
Where v$C>0$, $\Delta t_k$ is the time since the last event and $p_k \in \left\{-1,1\right\}$ is the polarity of the brightness change. From Eq.~\eqref{eq:event_generation_model}, the event generation model is dependent on:
...
\subsubsection{Event Input Representation}
